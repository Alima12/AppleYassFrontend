<template>
       ...
       <form action="" @submit.prevent="saveOffer()">
        ...
        <div class="row p-1 d-flex justify-content-between align-items-center" v-if="color != null">
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      axios.get(`config/super-offer/${code}/edit/`).then(response=>{
        this.offer = response.data
        this.color = this.offer.product;
        this.code = this.offer.product.product.code;
      })
    },
    methods:{
        isActive(date){
        ...
        saveOffer(){
            ...
            axios.put(`config/super-offer/${this.offer.id}/edit/`, data).then(response=>{
                Swal.fire({
                    title: 'موفق',
